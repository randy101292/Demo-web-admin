...
const useForm = (validate, act, callback) => {
  ...
  useEffect(() => {
    if (Object.keys(errors).length === 0 && isSubmitting) {
      let orig_time = values.time;
      values.time = values.time.value;

      if (action === "create") {
        request
          .post(
            `api/admin/service-center/${values.service_center_id}/time-slots`,
            values
          )
          .then((res) => {
      ...
      if (action === "update") {
        request
          .put(
            `api/admin/service-center/${values.service_center_id}/time-slots/${values.id}`,
            values
          )
          .then((res) => {

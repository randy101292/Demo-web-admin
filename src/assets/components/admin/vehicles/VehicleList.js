...
const VehicleList = () => {
  ...
  const deleteList = (row) => {
    ...
    // setSelectedRowIndex(row.index);
    request
      .delete(`api/admin/vehicle` + `/${row.original.id}`)
      .then((res) => {
  ...
  const onFinishLoading = (success, action) => {
    ...
    if (success && modalTitle === "Update Vehicle") {
      formRef.current.populateForm(selectedRow);
    } else if (success && modalTitle === "Delete Vehicle") {
  ...
  const fetchList = useCallback(async (props) => {
    ...
    try {
      const response = await request.get(`api/admin/vehicle`, {
        params: {
          page: pageIndex + 1,
          per_page: pageSize,
          //role: getSegment(2),
      ...
      setListData(response.vehicle.data);
      setLoading(false);
      setPageCount(response.vehicle.last_page);
    } catch (error) {
      console.log("catch-error", error);
      setListData([]);
      setLoading(false);
  ...
  return (
    <>
      ...
      >
        ...
        <CModalFooter>
          ...
          <CButton
            ...
            color={modalTitle === "Delete Vehicle" ? "danger" : "primary"}
            disabled={submitBtn}
          >
            {modalTitle === "Delete Vehicle" ? `Delete` : `Save changes`}
      ...
      <Table
        ...
        location={location}
        hideAction={
          !canAccess("edit vehicles") && !canAccess("delete vehicles")
            ? "act"
            : ""
        }
      />

...
import ServiceCenterTimeSlotForm from "./ServiceCenterTimeSlotForm";

const ServiceCenterTimeSlotList = () => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: "Max Limit",
        accessor: "max_limit", // accessor is the "key" in the data
        disableFilters: true,
      },
      {
        Header: "Date Created",
  ...
  const fetchList = useCallback(async (props) => {
    ...
    try {
      ...
      const response = await request.get(
        `api/admin/service-center/${params.service_center_id}`,
        {
          params: {
            page: pageIndex + 1,
            per_page: pageSize,
      ...
      console.log("response", response);
      setListData(response.service_center.timeslots);
      setLoading(false);
...
export default ServiceCenterTimeSlotList;

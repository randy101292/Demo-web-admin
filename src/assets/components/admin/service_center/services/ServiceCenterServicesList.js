...
import ServiceCenterServiceForm from "./ServiceCenterServiceForm";

const ServiceCenterServicesList = () => {
  ...
  const populateForm = (row) => {
    ...
    setModal(true);
    setModalTitle("Update Service Center Service");
  ...
  const deleteList = (row) => {
    request
      .delete(
        `api/admin/service-center/${params.service_center_id}/services` +
          `/${row.original.id}`
      )
      .then((res) => {
  ...
  const onFinishLoading = (success, action) => {
    ...
    if (success && modalTitle === "Update Service Center Service") {
      formRef.current.populateForm(selectedRow);
    } else if (success && modalTitle === "Delete Service Center Service") {
  ...
  const fetchList = useCallback(async (props) => {
    ...
    try {
      const response = await request.get(`api/admin/service-center-services`, {
        params: {
          page: pageIndex + 1,
          per_page: pageSize,
    ...
    <>
      <div className="d-flex justify-end">
        {canAccess("add services") ? (
          <CButton
            onClick={() => {
              setModalTitle("Create Service Center Service");
              setFormKey((val) => val + 1);
              setModal(true);
            }}
            color="secondary"
      ...
      >
        ...
        <CModalFooter>
          ...
          <CButton
            ...
            color={
              modalTitle === "Delete Service Center Service"
                ? "danger"
                : "primary"
            }
            disabled={submitBtn}
...
export default ServiceCenterServicesList;
